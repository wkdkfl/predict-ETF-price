...
- `공통데이터/`: 가격·금융지표·뉴스 헤드라인 수집 및 전처리
- `01_실패실험_수익률예측/`: 수익률 직접 예측 실험 (기각)
- `02_성공실험_변동성방향성예측/`: 변동성·방향성 예측 실험 (본 논문의 본 실험)
- `03_논문원고/`: 석사학위논문 최종 원고(docx)와 원고 생성 스크립트
- `plan/`, `doc/`: 작업 계획서와 발표 자료
